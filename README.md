...
- Uses `find` to locate MP3 files recursively.
- Runs `id3fix.py -f` on each discovered file.

## Notes

- `id3fix.py` uses heuristic repair and is intended for common encoding corruption patterns.
- It is not guaranteed to fix every case correctly, but it helps normalize many malformed ID3 tags to UTF-8.
